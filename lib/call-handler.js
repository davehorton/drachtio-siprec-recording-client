...
module.exports = (req, res) => {
  ...
  mrf.connect(config.get('freeswitch'))
    ...
    .then((opts) => {
      opts.uac.on('destroy', () => { opts.uas.destroy(); cleanup(opts); });
      opts.uas.on('destroy', () => { opts.uac.destroy(); cleanup(opts); });
      return ;
    })
    .catch((err) => {
...
function cleanup(opts) {
  if (!opts.cleaned) {
    ...
    [opts.epIncoming, opts.epOutgoing, opts.epSiprecCaller, opts.epSiprecCallee].forEach((ep) => {
      if (ep) ep.destroy();
    });
    if (opts.uacSiprec) opts.uacSiprec.destroy();
    if (opts.conf) opts.conf.destroy();
